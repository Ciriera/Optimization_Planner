...
async def create_admin_user(
    ...
    """
    Admin kullanıcısı oluştur (development için)
    """
    try:
        from app.models.user import User, UserRole
        
        # Önce admin kullanıcısının var olup olmadığını kontrol et
        existing_admin = await crud_user.get_by_email(db, email="admin@example.com")
        ...
        return {
            "success": False,
            "message": f"Admin kullanıcısı oluşturulamadı: {str(e)}"
        }
...
async def simple_login(
    ...
    """
    Basit login endpoint'i (development için)
    """
    try:
        # Default admin credentials for development
        if not login_data:
            login_data = LoginRequest(email="admin@example.com", password="admin123")
        
        print(f"Simple login attempt for user: {login_data.email}")
