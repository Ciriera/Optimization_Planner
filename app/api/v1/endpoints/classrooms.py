...
async def list_classrooms(
    ...
    try:
        ...
        result = await db.execute(text("SELECT * FROM classrooms LIMIT 10"))
        classrooms = result.fetchall()
        
        # Convert to list of dicts
        classroom_list = []
        for classroom in classrooms:
            classroom_dict = dict(classroom._mapping)
...
async def list_classrooms_public(
    db: AsyncSession = Depends(get_db)
) -> Any:
    """
    Public classrooms endpoint for frontend dashboard
    """
    try:
        from sqlalchemy import text
        result = await db.execute(text("SELECT * FROM classrooms"))
        classrooms = result.fetchall()
...
async def seed_default_classrooms(
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(check_admin_access),
) -> Any:
    """
    Default classroom'ları oluşturur (D106-D111, D223).
    """
    try:
        from app.services.classroom_seeder import ClassroomSeeder
        
        seeder = ClassroomSeeder()
        result = await seeder.seed_default_classrooms(db)
        
        return {
            "message": "Default classrooms seeded successfully",
            "result": result
        }
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Classroom seeding error: {str(e)}"
        )


@router.get("/validate-names")
async def validate_classroom_names(
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(deps.get_current_user),
) -> Any:
    """
    ...
    try:
        ...
        seeder = ClassroomSeeder()
        result = await seeder.validate_classroom_names(db)
        
        return result
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Classroom validation error: {str(e)}"
        )