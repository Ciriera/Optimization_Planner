"""
Firefly Algorithm - Enhanced with Pure Consecutive Grouping
...
class Firefly(OptimizationAlgorithm):
    """
    Firefly Algorithm - Enhanced with Pure Consecutive Grouping + Smart Jury Assignment.
    ...
    - Firefly light intensity optimization
    ...
    def __init__(self, params: Dict[str, Any] = None):
        ...
        # Firefly parameters
        self.n_fireflies = params.get("n_fireflies", 50)
        self.max_generations = params.get("max_generations", 100)
        self.alpha = params.get("alpha", 0.5)  # Randomness factor
        self.beta_min = params.get("beta_min", 0.2)  # Minimum attractiveness
        self.gamma = params.get("gamma", 1.0)  # Light absorption coefficient
    ...
    def optimize(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Run Firefly optimization with Pure Consecutive Grouping + Smart Jury.
        ...
        logger.info("Firefly Algorithm başlatılıyor (Enhanced Randomizer + Consecutive Grouping + Smart Jury mode)...")
        ...
        logger.info(f"Firefly Algorithm completed. Execution time: {execution_time:.2f}s")
        ...
        return {
            ...
            "algorithm": "Firefly Algorithm (Enhanced Randomizer + Consecutive Grouping + Smart Jury)",
            "status": "completed",
    ...
    def repair_solution(self, solution: Dict[str, Any], validation_report: Dict[str, Any]) -> Dict[str, Any]:
        """
        Firefly Algorithm icin ozel onarim metodlari.
        Firefly Algorithm ateş böceği optimizasyonu yaklaşımı kullanır.
        """
        assignments = solution.get("assignments", [])
        
        # Firefly Algorithm-specific repair: firefly optimization approach
        assignments = self._repair_duplicates_firefly(assignments)
        assignments = self._repair_gaps_firefly(assignments)
        assignments = self._repair_coverage_firefly(assignments)
        assignments = self._repair_light_constraints(assignments)
        
        solution["assignments"] = assignments
        return solution

    def _repair_duplicates_firefly(self, assignments):
        """Firefly Algorithm-specific duplicate repair using light intensity"""
        from collections import defaultdict
        
        # Group by project_id and keep the best assignment using light intensity
        ...
        # For each project, choose the best assignment using light intensity
        repaired = []
        for project_id, project_list in project_assignments.items():
            if len(project_list) == 1:
                repaired.append(project_list[0])
            else:
                # Light intensity selection: choose the assignment with best light
                best_assignment = self._light_select_best_assignment(project_list)
                repaired.append(best_assignment)
        
        return repaired

    def _repair_gaps_firefly(self, assignments):
        """Firefly Algorithm-specific gap repair using light intensity"""
        # Group by classroom
        ...
        for classroom_id, class_assignments in classroom_assignments.items():
            ...
            # Light intensity gap filling: use firefly behavior
            light_assignments = self._light_fill_gaps(sorted_assignments)
            repaired.extend(light_assignments)
        
        return repaired

    def _repair_coverage_firefly(self, assignments):
        """Firefly Algorithm-specific coverage repair ensuring all projects are assigned"""
        ...
        # Add missing projects with light intensity assignment
        for project_id in missing_projects:
            project = next((p for p in self.projects if p.get("id") == project_id), None)
            if project:
                # Find best available slot using light intensity
                best_slot = self._light_find_best_slot(project, assignments)
                if best_slot:
                    instructors = self._get_project_instructors_firefly(project)
                    if instructors:
                        new_assignment = {
    ...
    def _repair_light_constraints(self, assignments):
        """Firefly Algorithm-specific constraint repair ensuring light constraints"""
        # Remove assignments that violate light constraints
        repaired = []
        for assignment in assignments:
            timeslot_id = assignment.get("timeslot_id")
    ...
    def _light_select_best_assignment(self, assignments):
        """Light intensity selection of best assignment"""
        best_assignment = assignments[0]
        best_light = self._calculate_light_intensity(assignments[0])
        
        for assignment in assignments[1:]:
            light = self._calculate_light_intensity(assignment)
            if light > best_light:
                best_light = light
    ...
    def _calculate_light_intensity(self, assignment):
        """Calculate light intensity for an assignment"""
        score = 0
        timeslot_id = assignment.get("timeslot_id", "")
        classroom_id = assignment.get("classroom_id", "")
        
        # Prefer timeslots that have high light intensity
        try:
            hour = int(timeslot_id.split("_")[0]) if "_" in timeslot_id else 9
            # Light intensity: prefer timeslots that have high light
            if 9 <= hour <= 12:  # Morning light
                score += 30
            elif 13 <= hour <= 16:  # Afternoon light
                score += 25
            else:
                score += 10
        except:
        ...
        # Prefer classrooms that have high light intensity
        if "A" in classroom_id:
            score += 20  # A classrooms have high light
        elif "B" in classroom_id:
            score += 15  # B classrooms have high light
        
        return score

    def _light_fill_gaps(self, assignments):
        """Fill gaps using light intensity"""
        if len(assignments) <= 1:
            return assignments
        
        # Light intensity gap filling - keep all assignments for now
        return assignments

    def _light_find_best_slot(self, project, assignments):
        """Find best available slot using light intensity"""
        ...
        for classroom in self.classrooms:
            for timeslot in self.timeslots:
                ...
                if slot_key not in used_slots:
                    light = self._calculate_light_intensity({"timeslot_id": timeslot.get("id"), "classroom_id": classroom.get("id")})
                    if light > best_light:
                        best_light = light
                        best_slot = {
    ...
    def _get_project_instructors_firefly(self, project):
        """Get instructors for a project using light intensity"""
        instructors = []
        responsible_id = project.get("responsible_id")
        if responsible_id:
            instructors.append(responsible_id)
        
        # Add additional instructors based on project type (light: high intensity)
        project_type = project.get("type", "ara")
        if project_type == "bitirme":
            # Add jury members for light intensity
