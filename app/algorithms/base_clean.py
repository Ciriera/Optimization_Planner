...
class OptimizationAlgorithmWrapper:
    ...
    def optimize_with_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 4. Algoritma kendi repair'ini yapmalı - ortak repair kaldırıldı
    ...
    def _generate_validation_reports(self):
        ...
        try:
            ...
            final_dup = validator.detect_duplicates(self.validated_solution)
            final_cov = validator.detect_coverage(self.validated_solution, expected_ids)
            final_gaps = validator.detect_gaps(self.validated_solution, timeslots)
            final_late = validator.detect_late_slots(self.validated_solution, timeslots, late_pred)
            ...
            validator.write_json(os.path.join("reports", f"duplicate_report_{self.algorithm.get_name()}.json"), final_dup)
            validator.write_json(os.path.join("reports", f"coverage_report_{self.algorithm.get_name()}.json"), final_cov)
            validator.write_json(os.path.join("reports", f"gap_report_{self.algorithm.get_name()}.json"), final_gaps)
            validator.write_json(os.path.join("reports", f"late_slots_report_{self.algorithm.get_name()}.json"), final_late)
            validator.write_json(os.path.join("reports", f"load_balance_report_{self.algorithm.get_name()}.json"), {"per_instructor": dict(instr_load)})

            # Excel export
            try:
                out_path = os.path.join("reports", f"final_plan_{self.algorithm.get_name()}.xlsx")
                from app.algorithms.validator import export_schedule_to_excel
            ...
            try:
                ...
                from app.algorithms.validator import write_validator_summary
