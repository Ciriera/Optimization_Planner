"""
Particle Swarm Optimization Algorithm - Enhanced with Pure Consecutive Grouping
...
class PSO(OptimizationAlgorithm):
    """
    Particle Swarm Optimization Algorithm - Enhanced with Pure Consecutive Grouping + Smart Jury Assignment.
    ...
    def __init__(self, params: Dict[str, Any] = None):
        """
        Initialize PSO Algorithm with Pure Consecutive Grouping.
        ...
        # PSO parameters
        self.n_particles = params.get("n_particles", 50)
        self.n_iterations = params.get("n_iterations", 100)
        self.inertia_weight = params.get("inertia_weight", 0.7)
        self.cognitive_weight = params.get("cognitive_weight", 1.4)
        self.social_weight = params.get("social_weight", 1.4)

        # Gap-free assignment manager (for backwards compatibility)
    ...
    def optimize(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Pure Consecutive Grouping PSO Algorithm."""
        import time
        ...
        logger.info("Pure Consecutive Grouping PSO Algorithm başlatılıyor...")
        ...
        # Sadece consecutive grouping ile optimal çözüm oluştur
        logger.info("Consecutive grouping ile optimal çözüm oluşturuluyor...")
        
        # GapFreeAssignment manager'ı kullan
        gap_free_manager = GapFreeAssignment()
        
        assignments = gap_free_manager.assign_projects_gap_free(
            projects=self.projects,
            classrooms=self.classrooms,
            timeslots=self.timeslots,
            instructors=self.instructors
        )
        logger.info(f"  Consecutive Grouping: {len(assignments)} proje atandı")
        
        # Conflict detection ve resolution
        if assignments and len(assignments) > 0:
            logger.info("Conflict detection ve resolution...")
            
            conflicts = gap_free_manager.detect_instructor_conflicts(assignments, self.projects)
            
            if conflicts:
                logger.warning(f"  {len(conflicts)} conflict detected!")
                assignments = gap_free_manager.resolve_instructor_conflicts(
                ...
                remaining_conflicts = gap_free_manager.detect_instructor_conflicts(assignments, self.projects)
        ...
        quality = self.evaluate_fitness({"assignments": assignments})
        
        logger.info(f"Pure Consecutive Grouping PSO completed. Execution time: {execution_time:.2f}s")
        
        return {
            "schedule": assignments,
            "solution": assignments,  # Backward compatibility
            "assignments": assignments,
            "quality": quality,
            "execution_time": execution_time,
            "algorithm": "Pure Consecutive Grouping PSO",
            "status": "completed",
            "optimizations_applied": [
                "consecutive_grouping",
                "instructor_conflict_detection",
                "instructor_conflict_resolution"
            ],
            "parameters": {
                "algorithm_type": "consecutive_grouping_only",
    ...
    def repair_solution(self, solution: Dict[str, Any], validation_report: Dict[str, Any]) -> Dict[str, Any]:
        """
        PSO icin ozel onarim metodlari.
        PSO parçacık sürü optimizasyonu yaklaşımı kullanır.
        """
        assignments = solution.get("assignments", [])
        
        # PSO-specific repair: swarm intelligence approach
        assignments = self._repair_duplicates_pso(assignments)
        assignments = self._repair_gaps_pso(assignments)
        assignments = self._repair_coverage_pso(assignments)
        assignments = self._repair_swarm_constraints(assignments)
        
        solution["assignments"] = assignments
        return solution

    def _repair_duplicates_pso(self, assignments):
        """PSO-specific duplicate repair using swarm intelligence"""
        from collections import defaultdict
        
        # Group by project_id and keep the best assignment using swarm intelligence
        ...
        # For each project, choose the best assignment using swarm intelligence
        repaired = []
        for project_id, project_list in project_assignments.items():
            if len(project_list) == 1:
                repaired.append(project_list[0])
            else:
                # Swarm intelligence selection: choose the assignment with best swarm fitness
                best_assignment = self._swarm_select_best_assignment(project_list)
                repaired.append(best_assignment)
        
        return repaired

    def _repair_gaps_pso(self, assignments):
        """PSO-specific gap repair using swarm intelligence"""
        # Group by classroom
        ...
        for classroom_id, class_assignments in classroom_assignments.items():
            ...
            # Swarm intelligence gap filling: use particle behavior
            swarm_assignments = self._swarm_fill_gaps(sorted_assignments)
            repaired.extend(swarm_assignments)
        
        return repaired

    def _repair_coverage_pso(self, assignments):
        """PSO-specific coverage repair ensuring all projects are assigned"""
        ...
        # Add missing projects with swarm intelligence assignment
        for project_id in missing_projects:
            project = next((p for p in self.projects if p.get("id") == project_id), None)
            if project:
                # Find best available slot using swarm intelligence
                best_slot = self._swarm_find_best_slot(project, assignments)
                if best_slot:
                    instructors = self._get_project_instructors_pso(project)
                    if instructors:
                        new_assignment = {
    ...
    def _repair_swarm_constraints(self, assignments):
        """PSO-specific constraint repair ensuring swarm constraints"""
    ...
    def _swarm_select_best_assignment(self, assignments):
        """Swarm intelligence selection of best assignment"""
    ...
    def _swarm_fill_gaps(self, assignments):
        """Fill gaps using swarm intelligence"""
        if len(assignments) <= 1:
            return assignments
        
        # Swarm intelligence gap filling - keep all assignments for now
        return assignments
    ...
    def _get_project_instructors_pso(self, project):
        """Get instructors for a project using swarm intelligence"""
        instructors = []
        responsible_id = project.get("responsible_id")
        if responsible_id:
            instructors.append(responsible_id)
    ...
    def execute(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info("PSO Algorithm başlatılıyor (Enhanced Randomizer + Consecutive Grouping + Smart Jury mode)...")
        logger.info(f"  Projeler: {len(self.projects)}")
        logger.info(f"  Instructors: {len(self.instructors)}")
        logger.info(f"  Sınıflar: {len(self.classrooms)}")
        logger.info(f"  Zaman Slotları: {len(self.timeslots)}")
        ...
        execution_time = end_time - start_time
        logger.info(f"PSO Algorithm completed. Execution time: {execution_time:.2f}s")
        ...
        return {
            ...
            "execution_time": execution_time,
            "algorithm": "PSO Algorithm (Enhanced Randomizer + Consecutive Grouping + Smart Jury)",
            "status": "completed",
            "optimizations_applied": [
