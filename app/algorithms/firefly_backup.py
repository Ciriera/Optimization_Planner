"""
Firefly (Atesbocegi) algoritmasi sinifi - CP-SAT ozellikli versiyon.
"""
...
class Firefly(OptimizationAlgorithm):
    """
    Firefly (Atesbocegi) algoritmasi sinifi.
    Atesbocegi algoritmasi kullanarak proje atama problemini cozer.
    """
...
def __init__(self, params: Dict[str, Any] = None):
        """
        Firefly algoritmasi baslatici - CP-SAT ozellikli versiyon.
    ...
    def _select_instructors_for_project_gap_free(self, project: Dict[str, Any], instructor_timeslot_usage: Dict[int, Set[int]]) -> List[int]:
        ...
        self.beta0 = params.get("beta0", 1.0)  # Cekicilik parametresi
        self.gamma = params.get("gamma", 1.0)  # Isik emilim katsayisi
    ...
    def initialize(self, data: Dict[str, Any]) -> None:
        """
        Firefly algoritmasini baslangic verileri ile baslatir - CP-SAT ozellikli versiyon.
        ...
        # Atesboceklerini baslat
        self.fireflies = self._initialize_fireflies()
    
    def optimize(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Firefly algoritmasini calistirir - CP-SAT ozellikli versiyon.
        ...
        initial_firefly = self._create_initial_solution()
        initial_fitness = self.evaluate_fitness({"solution": initial_firefly})

        # En iyi cozumu guncelle (CP-SAT ozelligi)
        self.best_solution = initial_firefly
        ...
        while time.time() - start_time < self.time_limit and iteration < self.n_iterations:
            ...
            for i in range(self.n_fireflies):
                # Diger atesbocekleriyle karsilastir
                for j in range(self.n_fireflies):
                    # Eger j atesbocegi i'den daha parlaksa (daha iyi uygunluk degerine sahipse)
                    if self.fireflies[j]["fitness"] > self.fireflies[i]["fitness"]:
                        # i atesbocegini j'ye dogru hareket ettir
                        self._move_firefly(i, j)
                        
                        # i atesboceginin uygunlugunu yeniden degerlendir
            ...
            # Alpha parametresini azalt (sogutma)
            self.alpha *= 0.97
    ...
    def _initialize_fireflies(self) -> List[Dict[str, Any]]:
        """
        Atesboceklerini baslatir.
        
        Returns:
            List[Dict[str, Any]]: Baslangic atesbocekleri.
        """
        fireflies = []
    ...
    def _move_firefly(self, i: int, j: int) -> None:
        """
        i atesbocegini j atesbocegine dogru hareket ettirir.
        
        Args:
            i: Hareket eden atesbocegi indeksi.
            j: Ceken atesbocegi indeksi.
        """
        # Cozumler arasi mesafeyi hesapla
    ...
    def _calculate_distance(self, solution1: List[Dict[str, Any]], solution2: List[Dict[str, Any]]) -> float:
        """
        Iki cozum arasindaki mesafeyi hesaplar.
        
        Args:
            solution1: Birinci cozum.
            solution2: Ikinci cozum.
            
        Returns:
            float: Iki cozum arasindaki mesafe.
        """
        # Basit bir mesafe metrigi: Farkli atamalarin orani
        ...
        # Farkli atamalari say
        differences = 0
        
        for i in range(min_length):
            # Sinif ve zaman dilimi farkliysa
    ...
    def _generate_new_position(self, solution1: List[Dict[str, Any]], solution2: List[Dict[str, Any]], beta: float) -> List[Dict[str, Any]]:
        """
        Iki cozumden yeni bir cozum olusturur.
        
        Args:
            solution1: Birinci cozum (hareket eden).
            solution2: Ikinci cozum (ceken).
            beta: Cekicilik degeri.
            
        Returns:
            List[Dict[str, Any]]: Yeni cozum.
        """
        # Yeni cozum icin temel olarak solution1'i kullan
        new_solution = copy.deepcopy(solution1)
        
        # Cozumleri proje ID'lerine gore sirala
        sorted_solution2 = sorted(solution2, key=lambda x: x["project_id"])
        
        # Beta olasiligiyla solution2'den atamalar al
        for i, assignment in enumerate(new_solution):
            if random.random() < beta:
                # Ayni projeye ait atamayi solution2'den bul
                project_id = assignment["project_id"]
                s2_assignment = next((a for a in sorted_solution2 if a.get("project_id") == project_id), None)
                
                if s2_assignment:
                    # Beta olasiligiyla sinifi al
                    if random.random() < beta:
                        assignment["classroom_id"] = s2_assignment["classroom_id"]
                    
                    # Beta olasiligiyla zaman dilimini al
                    if random.random() < beta:
                        assignment["timeslot_id"] = s2_assignment["timeslot_id"]
                    
                    # Beta olasiligiyla katilimcilari al
                    if random.random() < beta:
                        assignment["instructors"] = copy.deepcopy(s2_assignment["instructors"])
        
        # Alpha olasiligiyla rastgele degisiklikler yap
