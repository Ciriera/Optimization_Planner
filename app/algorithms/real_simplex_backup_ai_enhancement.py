...
🎯 CORE STRATEGY - Earliest-First Instructor Pairing & Consecutive Grouping:
═══════════════════════════════════════════════════════════════════════════════
...
STEP 4: Earliest-First Consecutive Grouping + Bi-Directional Jury
─────────────────────────────────────────────────────────────────
- ⭐ HER PAIR EN ERKEN BOŞ SAATTEN BAŞLAR (earliest-first strategy)
...
1. ⭐ EARLIEST-FIRST STRATEGY - Always fill earliest available timeslots first
2. PROJECT-BASED INSTRUCTOR SORTING - Sort by project responsibility count (max → min)
3. SMART GROUPING & PAIRING - Split instructors into balanced groups and pair them
4. CONSECUTIVE GROUPING - All projects of an instructor in same classroom, consecutive slots
5. BI-DIRECTIONAL JURY ASSIGNMENT - x supervises → y jury, then y supervises → x jury
6. CONFLICT-AWARE OPERATIONS - All assignments and swaps check for conflicts
7. AI-BASED SCORING - Reward system instead of hard constraints
8. NO HARD CONSTRAINTS - Everything is optimized, nothing is blocked
"""
...
class RealSimplexAlgorithm(OptimizationAlgorithm):
    ...
    5. CONSECUTIVE GROUPING - Each instructor's projects in same classroom, consecutive
    6. BI-DIRECTIONAL JURY - Paired instructors are each other's jury (x→y, y→x)
    7. EARLY TIMESLOT OPTIMIZATION - Prioritize early timeslots over late ones
    8. GAP-FREE OPTIMIZATION - AI-based gap elimination with advanced repositioning
    9. CONFLICT-AWARE OPERATIONS - All assignments and swaps check for conflicts
    10. SOFT CONSTRAINTS - Everything is optimized, nothing is blocked
    11. AI-DRIVEN SCORING - Reward good patterns, penalize bad ones
    """
    ...
    def __init__(self, params: Dict[str, Any] = None):
        ...
        self.reward_early_timeslot = 200.0      # Early timeslot usage (prioritize morning - INCREASED)
        ...
        self.penalty_late_timeslot = -300.0     # Late timeslot usage (force early slots - INCREASED)
    ...
    def optimize(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Phase 4: Priority Early Timeslot Optimization (15:30-17:00)
        logger.info("")
        logger.info("🎯 Phase 4: Priority Early Timeslot Optimization (15:30-17:00)")
        assignments = self._optimize_priority_early_timeslots(assignments)
        logger.info(f"✅ Priority early timeslot optimization tamamlandı")
        
        # Phase 4.5: General Early Timeslot Optimization (AI-Based)
        logger.info("")
        logger.info("🕐 Phase 4.5: General Early Timeslot Optimization (AI-Based)")
        assignments = self._optimize_early_timeslots(assignments)
        logger.info(f"✅ General early timeslot optimization tamamlandı")
    ...
    def _create_paired_consecutive_solution(self) -> List[Dict[str, Any]]:
        """
        Create paired consecutive solution with EARLIEST-FIRST & CONSECUTIVE GROUPING.
        
        ═══════════════════════════════════════════════════════════════════════════════
        EARLIEST-FIRST CONSECUTIVE GROUPING STRATEGY
        ═══════════════════════════════════════════════════════════════════════════════
        ...
        1️⃣ ASSIGN X's PROJECTS CONSECUTIVELY (EARLIEST FIRST)
           ...
           - ⭐ START FROM THE EARLIEST AVAILABLE SLOT (not from where previous pair ended)
        ...
        for pair_idx, (inst_a_id, inst_b_id) in enumerate(self.instructor_pairs):
            ...
            # ───────────────────────────────────────────────────────────────────────────
            # STEP 1: Assign Instructor A's projects CONSECUTIVELY (EARLIEST FIRST)
            # ───────────────────────────────────────────────────────────────────────────
            ...
            for proj_idx, project in enumerate(inst_a_projects, 1):
                ...
                if use_consecutive:
                    ...
                    # Non-consecutive: Find any available slot
                    while current_idx < len(sorted_timeslots):
                        timeslot_id = sorted_timeslots[current_idx].get("id")
                        # More flexible - allow any slot
                        if timeslot_id not in instructor_timeslot_usage[inst_a_id]:
                            break
                        current_idx += 1
    ...
    def _find_earliest_available_slot_index(
        self,
        classroom_id: int,
        sorted_timeslots: List[Dict[str, Any]],
        slot_usage: Dict,
        ...
        for idx, timeslot in enumerate(sorted_timeslots):
            ...
            # Check if instructor is free at this slot (hard constraint)
            if timeslot_id not in instructor_used_slots:
                # Prefer completely unused slots
                if slot_usage.get(slot_key, 0) == 0:
                    return idx
        
        # If no perfect slot, find first slot where instructor is free (allow slot reuse)
        for idx, timeslot in enumerate(sorted_timeslots):
            timeslot_id = timeslot.get("id")
        ...
        # Fallback: return first slot (soft constraint)
        return 0
    ...
    def _find_next_available_slot_index(
        ...
        For earliest-first strategy, use _find_earliest_available_slot_index instead.
        """
        for idx, timeslot in enumerate(sorted_timeslots):
            timeslot_id = timeslot.get("id")
            slot_key = (classroom_id, timeslot_id)
    ...
    def _optimize_priority_early_timeslots(self, assignments: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Priority Early Timeslot Optimization - Specifically target 15:30-17:00 slots.
        
        Strategy:
        1. Identify all projects scheduled after 17:00
        2. Find available slots in 15:30-17:00 range
        3. Aggressively move late projects to priority early slots
        4. Use maximum scoring for 15:30-17:00 slots
        """
        logger.info("🎯 Priority Early Timeslot Optimization (15:30-17:00) başlatılıyor...")
        
        # Get all timeslots sorted by time
        ...
        logger.info(f"   🎯 Öncelikli slotlar (15:30-17:00): {len(priority_early_timeslots)}")
        
        # Find projects scheduled after 17:00
        late_projects = []
        for assignment in assignments:
            timeslot_id = assignment.get("timeslot_id")
            if timeslot_id:
        ...
        logger.info(f"   📋 17:00 sonrası projeler: {len(late_projects)}")
        
        if not late_projects:
            logger.info("   ✅ 17:00 sonrası proje yok, optimizasyon gerekmiyor")
            return assignments
        
        # Find available priority early slots
        used_priority_slots = set()
        for assignment in assignments:
            timeslot_id = assignment.get("timeslot_id")
            if timeslot_id:
                timeslot = next((ts for ts in self.timeslots if ts["id"] == timeslot_id), None)
                if timeslot and timeslot in priority_early_timeslots:
                    used_priority_slots.add(timeslot_id)
        
        available_priority_slots = [
            ts["id"] for ts in priority_early_timeslots 
            if ts["id"] not in used_priority_slots
        ]
        
        logger.info(f"   🆓 Öncelikli slotlarda boş yer: {len(available_priority_slots)}")
        
        # Aggressively move late projects to priority early slots
        moved_count = 0
        
        for late_project in late_projects:
            if not available_priority_slots:
                break
            
            current_timeslot_id = late_project.get("timeslot_id")
            current_classroom_id = late_project.get("classroom_id")
            
            # Find best priority early slot
            best_priority_slot = None
            best_score = float('-inf')
            
            for priority_slot_id in available_priority_slots:
                # Calculate score for this move (extra high reward for priority slots)
                score = self._calculate_priority_timeslot_score(
                    late_project, current_classroom_id, priority_slot_id
                )
                
                if score > best_score:
                    best_score = score
                    best_priority_slot = priority_slot_id
            
            # Move to priority early slot
            if best_priority_slot:
                late_project["timeslot_id"] = best_priority_slot
                available_priority_slots.remove(best_priority_slot)
                moved_count += 1
                
                # Get timeslot info for logging
                timeslot = next((ts for ts in self.timeslots if ts["id"] == best_priority_slot), None)
                new_start_time = timeslot.get("start_time", "unknown") if timeslot else "unknown"
                
                logger.info(
                    f"    🎯 PRIORITY MOVE: Proje {late_project['project_id']} öncelikli slota taşındı: "
                    f"Slot {current_timeslot_id} → {best_priority_slot} ({new_start_time}) (Score: {best_score:.1f})"
                )
        
        logger.info(f"✅ Priority Early Timeslot Optimization: {moved_count} proje öncelikli slotlara taşındı")
        return assignments
    
    def _calculate_priority_timeslot_score(self, project: Dict[str, Any], classroom_id: int, timeslot_id: int) -> float:
        """
        ...
        if timeslot:
            start_time = timeslot.get("start_time", "")
            logger.debug(f"   🎯 Priority slot {start_time}: +{self.reward_early_timeslot * 3.0}")
        
        # Very soft penalty for conflicts (we want to use these slots aggressively)
        instructor_id = project.get("instructors", [None])[0]
        if instructor_id:
            score += self.penalty_conflict * 0.05  # Even softer penalty
        
        # Check classroom conflicts (soft)
        conflicts = 0
        ...
        if conflicts > 0:
            score += self.penalty_conflict * conflicts * 0.3  # Soft penalty
    ...
    def _optimize_early_timeslots(self, assignments: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        2. Find early timeslots that are available (15:30-17:00 priority)
        ...
        # Define priority early timeslots (15:30-17:00 range)
        priority_early_timeslots = []
        regular_early_timeslots = []
        
        for ts in sorted_timeslots:
            start_time = ts.get("start_time", "")
            # Check if timeslot is in the priority range (15:30-17:00)
            if "15:30" <= start_time <= "17:00":
                priority_early_timeslots.append(ts)
            elif start_time < "15:30":
                regular_early_timeslots.append(ts)
        
        # Combine priority and regular early timeslots (priority first)
        early_timeslots = priority_early_timeslots + regular_early_timeslots
        early_threshold = len(early_timeslots)
        late_timeslots = [ts for ts in sorted_timeslots if ts not in early_timeslots]
        
        logger.info(f"   🎯 Öncelikli erken slotlar (15:30-17:00): {len(priority_early_timeslots)}")
        logger.info(f"   📅 Normal erken slotlar: {len(regular_early_timeslots)}")
        logger.info(f"   📅 Toplam erken slotlar: {len(early_timeslots)}")
        logger.info(f"   📅 Geç slotlar: {len(late_timeslots)}")
    ...
    def _calculate_early_timeslot_score(self, project: Dict[str, Any], classroom_id: int, timeslot_id: int) -> float:
        ...
        Prioritizes 15:30-17:00 time slots as requested.
        """
        score = 0.0
        
        # Get timeslot info to check if it's in priority range
        timeslot = next((ts for ts in self.timeslots if ts["id"] == timeslot_id), None)
        if timeslot:
            start_time = timeslot.get("start_time", "")
            
            # EXTRA HIGH reward for priority timeslots (15:30-17:00)
            if "15:30" <= start_time <= "17:00":
                score += self.reward_early_timeslot * 2.0  # Double reward for priority slots
                logger.debug(f"   🎯 Priority slot bonus for {start_time}: +{self.reward_early_timeslot * 2.0}")
            else:
                # Regular reward for other early timeslots
    ...
    def _assign_missing_projects(
        ...
        for instructor_id, projects_list in instructor_missing_projects.items():
            ...
            # 🔧 SORUN DÜZELTİLDİ: Consecutive slot bulunamadığında gelişmiş optimizasyon
            if len(consecutive_slots) < len(projects_sorted):
                logger.warning(
                    f"      ⚠️ Could only find {len(consecutive_slots)} consecutive slots "
                    f"for {len(projects_sorted)} projects. Using ADVANCED OPTIMIZATION..."
                )
                
                # GELİŞMİŞ OPTİMİZASYON: Mevcut atamaları optimize ederek consecutive slot yarat
                remaining_projects = projects_sorted[len(consecutive_slots):]
                logger.info(f"      🔄 OPTIMIZATION: {len(remaining_projects)} proje için consecutive slot yaratılıyor...")
                
                # 1. Önce mevcut atamaları optimize et
                optimized_slots = self._optimize_slots_for_consecutive(
                    best_classroom, remaining_projects, sorted_timeslots, 
                    slot_usage, instructor_timeslot_usage, existing_assignments
                )
                
                if optimized_slots:
                    logger.info(f"      ✅ OPTIMIZATION: {len(optimized_slots)} consecutive slot yaratıldı!")
                    consecutive_slots.extend(optimized_slots)
                else:
                    # 2. Optimizasyon başarısızsa fallback
                    logger.warning(f"      🔄 FALLBACK: Optimizasyon başarısız, herhangi bir boş slot aranıyor...")
                    
                    for project in remaining_projects:
                        project_id = project.get("id")
                        project_type = project.get("type", "ara")
                        
                        # Herhangi bir boş slot bul
                        fallback_slot = self._find_any_available_slot(
                            best_classroom, sorted_timeslots, slot_usage, 
                            instructor_timeslot_usage[instructor_id]
                        )
                        ...
                        if fallback_slot:
                            ...
                            slot_usage[(best_classroom, fallback_slot)] += 1
                            instructor_timeslot_usage[instructor_id].add(fallback_slot)
                            classroom_timeslot_usage[best_classroom].add(fallback_slot)
                            
                            logger.info(f"        ✅ FALLBACK: Proje {project_id} ({project_type}) -> Slot {fallback_slot}")
                        else:
                            logger.error(f"        ❌ FALLBACK FAILED: Proje {project_id} için hiç boş slot bulunamadı!")
    ...
    def _find_any_available_slot(
        ...
        """
        Herhangi bir boş slot bul (consecutive olması gerekmez).
        
        Args:
            classroom_id: Sınıf ID
            sorted_timeslots: Sıralanmış zaman slotları
            slot_usage: Slot kullanım durumu
            instructor_used_slots: Öğretim üyesinin kullandığı slotlar
            
        Returns:
            Boş slot ID'si veya None
        """
        for timeslot in sorted_timeslots:
            timeslot_id = timeslot.get("id")
            slot_key = (classroom_id, timeslot_id)
            ...
            if (slot_key not in slot_usage and 
                timeslot_id not in instructor_used_slots):
                return timeslot_id
        
        return None
    
    def _optimize_slots_for_consecutive(
        self,
        classroom_id: int,
        ...
        """
        Mevcut atamaları optimize ederek consecutive slot yarat.
        
        Stratejiler:
        1. Mevcut atamaları başka sınıflara taşı
        2. Mevcut atamaları başka zaman slotlarına taşı
        3. Küçük yer değişimleri ile consecutive slot yarat
        
        Args:
            classroom_id: Hedef sınıf
            remaining_projects: Atanacak projeler
            sorted_timeslots: Sıralanmış zaman slotları
            slot_usage: Slot kullanım durumu
            instructor_timeslot_usage: Öğretim üyesi slot kullanımı
            existing_assignments: Mevcut atamalar
            
        Returns:
            Consecutive slot ID'leri listesi
        """
        logger.info(f"      🔄 OPTIMIZATION: Sınıf {classroom_id} için consecutive slot yaratılıyor...")
        
        needed_slots = len(remaining_projects)
        if needed_slots <= 0:
            return []
        
        # 1. STRATEJİ: Mevcut atamaları başka sınıflara taşı
        optimized_slots = self._relocate_assignments_for_consecutive(
            classroom_id, needed_slots, sorted_timeslots, 
            slot_usage, instructor_timeslot_usage, existing_assignments
        )
        
        if optimized_slots:
            logger.info(f"      ✅ RELOCATION: {len(optimized_slots)} consecutive slot yaratıldı!")
            return optimized_slots
        
        # 2. STRATEJİ: Küçük yer değişimleri ile consecutive slot yarat
        optimized_slots = self._swap_assignments_for_consecutive(
            classroom_id, needed_slots, sorted_timeslots, 
            slot_usage, instructor_timeslot_usage, existing_assignments
        )
        
        if optimized_slots:
            logger.info(f"      ✅ SWAP: {len(optimized_slots)} consecutive slot yaratıldı!")
            return optimized_slots
        
        # 3. STRATEJİ: Mevcut atamaları başka zaman slotlarına taşı
        optimized_slots = self._reschedule_assignments_for_consecutive(
            classroom_id, needed_slots, sorted_timeslots, 
            slot_usage, instructor_timeslot_usage, existing_assignments
        )
        
        if optimized_slots:
            logger.info(f"      ✅ RESCHEDULE: {len(optimized_slots)} consecutive slot yaratıldı!")
            return optimized_slots
        
        logger.warning(f"      ⚠️ OPTIMIZATION FAILED: Consecutive slot yaratılamadı")
        return []
    
    def _relocate_assignments_for_consecutive(
        self,
        classroom_id: int,
        needed_slots: int,
        sorted_timeslots: List[Dict[str, Any]],
        ...
        """
        Mevcut atamaları başka sınıflara taşıyarak consecutive slot yarat.
        """
        logger.info(f"        🔄 RELOCATION: Mevcut atamalar başka sınıflara taşınıyor...")
        
        # Bu sınıftaki mevcut atamaları bul
        ...
        # En az önemli atamaları bul (taşınabilir olanlar)
        relocatable_assignments = self._find_relocatable_assignments(classroom_assignments)
        
        if not relocatable_assignments:
            return []
        
        # Consecutive slot yaratmak için yeterli alan var mı?
        consecutive_slots = self._find_consecutive_available_slots(
            classroom_id, sorted_timeslots, 
            {k: v for k, v in slot_usage.items() if k[0] != classroom_id},  # Bu sınıf hariç
            instructor_timeslot_usage, needed_slots, set()
        )
        
        if len(consecutive_slots) >= needed_slots:
            logger.info(f"        ✅ RELOCATION: {len(consecutive_slots)} consecutive slot bulundu!")
            return consecutive_slots[:needed_slots]
        
        return []
    
    def _find_relocatable_assignments(self, assignments: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Taşınabilir atamaları bul (düşük öncelikli, başka sınıflarda yer olan).
        """
        relocatable = []
        
        for assignment in assignments:
            # Düşük öncelikli projeler (ara projeler, makeup projeler)
            project_type = assignment.get("project_type", "ara")
            is_makeup = assignment.get("is_makeup", False)
            
            if project_type == "ara" or is_makeup:
                relocatable.append(assignment)
        
        return relocatable
    
    def _swap_assignments_for_consecutive(
        self,
        classroom_id: int,
        needed_slots: int,
        sorted_timeslots: List[Dict[str, Any]],
        ...
        """
        Küçük yer değişimleri ile consecutive slot yarat.
        """
        logger.info(f"        🔄 SWAP: Küçük yer değişimleri ile consecutive slot yaratılıyor...")
        ...
        # En az önemli atamaları bul ve yer değiştir
        for assignment in classroom_assignments:
            current_timeslot = assignment.get("timeslot_id")
            if not current_timeslot:
                continue
            
            # Bu atamayı başka bir sınıfa taşı
            for other_classroom in self.classrooms:
                other_classroom_id = other_classroom.get("id")
                if other_classroom_id == classroom_id:
                    continue
                
                # Diğer sınıfta boş slot var mı?
                for timeslot in sorted_timeslots:
                    timeslot_id = timeslot.get("id")
                    slot_key = (other_classroom_id, timeslot_id)
                    ...
                    if slot_key not in slot_usage:
                        ...
                        # Şimdi consecutive slot var mı kontrol et
                        consecutive_slots = self._find_consecutive_available_slots(
                            classroom_id, sorted_timeslots, slot_usage, 
                            instructor_timeslot_usage, needed_slots, set()
                        )
                        
                        if len(consecutive_slots) >= needed_slots:
                            logger.info(f"        ✅ SWAP: {len(consecutive_slots)} consecutive slot yaratıldı!")
                            return consecutive_slots[:needed_slots]
                        
                        # Başarısızsa geri al
                        assignment["classroom_id"] = classroom_id
                        assignment["timeslot_id"] = current_timeslot
        
        return []
    
    def _reschedule_assignments_for_consecutive(
        self,
        classroom_id: int,
        needed_slots: int,
        sorted_timeslots: List[Dict[str, Any]],
        ...
        """
        Mevcut atamaları başka zaman slotlarına taşıyarak consecutive slot yarat.
        """
        logger.info(f"        🔄 RESCHEDULE: Mevcut atamalar başka zaman slotlarına taşınıyor...")
        ...
        # En az önemli atamaları bul ve zaman değiştir
        for assignment in classroom_assignments:
            current_timeslot = assignment.get("timeslot_id")
            if not current_timeslot:
                continue
            
            # Bu atamayı başka bir zaman slotuna taşı
            for timeslot in sorted_timeslots:
                timeslot_id = timeslot.get("id")
                ...
                if slot_key not in slot_usage:
                    ...
                    if len(consecutive_slots) >= needed_slots:
                        logger.info(f"        ✅ RESCHEDULE: {len(consecutive_slots)} consecutive slot yaratıldı!")
                        return consecutive_slots[:needed_slots]
