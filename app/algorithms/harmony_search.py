"""
Harmony Search Algorithm - Gap-Free Version
"""
...
class HarmonySearch(OptimizationAlgorithm):
    """
    Harmony Search Algorithm - Enhanced with Pure Consecutive Grouping + Smart Jury Assignment.
    ...
    def __init__(self, params: Dict[str, Any] = None):
        ...
        self.harmony_memory_size = params.get("harmony_memory_size", 50)
        self.max_improvisations = params.get("max_improvisations", 1000)
        self.harmony_memory_consideration_rate = params.get("harmony_memory_consideration_rate", 0.95)
        self.pitch_adjustment_rate = params.get("pitch_adjustment_rate", 0.7)
        self.bandwidth = params.get("bandwidth", 0.1)
    ...
    def optimize(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Run Harmony Search optimization with Pure Consecutive Grouping + Smart Jury.
        ...
        logger.info("Harmony Search Algorithm başlatılıyor (Enhanced Randomizer + Consecutive Grouping + Smart Jury mode)...")
        ...
        logger.info(f"Harmony Search Algorithm completed. Execution time: {execution_time:.2f}s")
        ...
        return {
            ...
            "algorithm": "Harmony Search Algorithm (Enhanced Randomizer + Consecutive Grouping + Smart Jury)",
            "status": "completed",
            ...
            "parameters": {
                ...
                "max_improvisations": self.max_improvisations
            }
        }
    ...
    def repair_solution(self, solution: Dict[str, Any], validation_report: Dict[str, Any]) -> Dict[str, Any]:
        """
        Harmony Search icin ozel onarim metodlari.
        Harmony Search müziksel uyum yaklaşımı kullanır.
        """
        assignments = solution.get("assignments", [])
        
        # Harmony Search-specific repair: musical harmony approach
        assignments = self._repair_duplicates_harmony(assignments)
        assignments = self._repair_gaps_harmony(assignments)
        assignments = self._repair_coverage_harmony(assignments)
        assignments = self._repair_harmony_constraints(assignments)
        
        solution["assignments"] = assignments
        return solution

    def _repair_duplicates_harmony(self, assignments):
        """Harmony Search-specific duplicate repair using musical harmony"""
        from collections import defaultdict
        
        # Group by project_id and keep the most harmonious assignment
        ...
        # For each project, choose the most harmonious assignment
        repaired = []
        for project_id, project_list in project_assignments.items():
            if len(project_list) == 1:
                repaired.append(project_list[0])
            else:
                # Musical harmony selection: choose the assignment with best harmony
                best_assignment = self._harmony_select_best_assignment(project_list)
                repaired.append(best_assignment)
        
        return repaired

    def _repair_gaps_harmony(self, assignments):
        """Harmony Search-specific gap repair using musical harmony"""
        # Group by classroom
        ...
        for classroom_id, class_assignments in classroom_assignments.items():
            ...
            # Musical harmony gap filling: create harmonious sequences
            harmonious_assignments = self._create_harmonious_sequence(sorted_assignments)
            repaired.extend(harmonious_assignments)
        
        return repaired

    def _repair_coverage_harmony(self, assignments):
        """Harmony Search-specific coverage repair ensuring all projects are assigned"""
        ...
        # Add missing projects with harmonious assignment
        for project_id in missing_projects:
            project = next((p for p in self.projects if p.get("id") == project_id), None)
            if project:
                # Find most harmonious available slot
                harmonious_slot = self._find_harmonious_slot(project, assignments)
                if harmonious_slot:
                    instructors = self._get_project_instructors_harmony(project)
                    if instructors:
                        new_assignment = {
                            "project_id": project_id,
                            "classroom_id": harmonious_slot["classroom_id"],
                            "timeslot_id": harmonious_slot["timeslot_id"],
    ...
    def _repair_harmony_constraints(self, assignments):
        """Harmony Search-specific constraint repair ensuring musical harmony"""
        # Remove assignments that violate harmony constraints
        repaired = []
        for assignment in assignments:
            timeslot_id = assignment.get("timeslot_id")
    ...
    def _harmony_select_best_assignment(self, assignments):
        """Harmony selection of best assignment based on musical harmony"""
        best_assignment = assignments[0]
        best_harmony = self._calculate_harmony_score(assignments[0])
        
        for assignment in assignments[1:]:
            harmony = self._calculate_harmony_score(assignment)
    ...
    def _calculate_harmony_score(self, assignment):
        """Calculate harmony score for an assignment"""
        score = 0
        timeslot_id = assignment.get("timeslot_id", "")
        classroom_id = assignment.get("classroom_id", "")
        
        # Prefer harmonious timeslots (musical harmony)
        try:
            hour = int(timeslot_id.split("_")[0]) if "_" in timeslot_id else 9
            # Musical harmony: prefer certain hours that create harmony
            if 9 <= hour <= 12:  # Morning harmony
                score += 30
            elif 13 <= hour <= 16:  # Afternoon harmony
                score += 25
            else:
                score += 10
        except:
        ...
        # Prefer harmonious classrooms (musical harmony)
        if "A" in classroom_id:
            score += 20  # A classrooms create harmony
        elif "B" in classroom_id:
            score += 15  # B classrooms create harmony
        
        return score

    def _create_harmonious_sequence(self, assignments):
        """Create harmonious sequence of assignments"""
        if len(assignments) <= 1:
            return assignments
        
        # Musical harmony: create sequences that flow well
        return assignments

    def _find_harmonious_slot(self, project, assignments):
        """Find most harmonious available slot"""
        ...
        for classroom in self.classrooms:
            for timeslot in self.timeslots:
                ...
                if slot_key not in used_slots:
                    harmony = self._calculate_harmony_score({"timeslot_id": timeslot.get("id"), "classroom_id": classroom.get("id")})
                    if harmony > best_harmony:
                        best_harmony = harmony
                        best_slot = {
    ...
    def _get_project_instructors_harmony(self, project):
        """Get instructors for a project using harmony approach"""
        instructors = []
        responsible_id = project.get("responsible_id")
        if responsible_id:
            instructors.append(responsible_id)
        
        # Add additional instructors based on project type (harmony: musical balance)
        project_type = project.get("type", "ara")
        if project_type == "bitirme":
            # Add jury members for harmony
