...
class AlgorithmService:
    ...
    ALGORITHM_DETAILS = {
        ...
        "hybrid_cpsat_tabu_nsga": {
            "name": "Hybrid CP-SAT + Tabu + NSGA-II",
            "description": "A sophisticated hybrid algorithm that combines CP-SAT for feasibility, Tabu Search for local optimization, Simulated Annealing for global optimization, and NSGA-II for multi-objective optimization.",
            "strengths": [
                "Combines multiple optimization techniques",
                "Guarantees feasibility with CP-SAT",
            ...
            "parameters": {
                "time_limit": "Total execution time limit in seconds",
                "cp_time_limit": "CP-SAT phase time limit in seconds",
                "tabu_iterations": "Number of Tabu Search iterations",
                "sa_iterations": "Number of Simulated Annealing iterations",
                "nsga_generations": "Number of NSGA-II generations",
        ...
        "lexicographic_advanced": {
            "name": "Advanced Lexicographic Optimization",
            "description": "A mathematically rigorous approach using CP-SAT with lexicographic optimization to solve multi-objective problems in a hierarchical manner.",
            "strengths": [
                "Mathematically optimal solutions",
                "Systematic multi-objective approach",
                "Guaranteed feasibility",
                "Clear priority ordering"
            ],
            "weaknesses": [
                "May be computationally expensive",
                "Requires careful objective ordering",
                "Limited flexibility in objective weights"
            ],
            "parameters": {
                "time_limit_seconds": "Total execution time limit in seconds",
                "stage_time_limit": "Time limit per optimization stage in seconds",
...
def run_algorithm(
    ...
    try:
        ...
        algorithm_run.started_at = datetime.now()
        db.commit()
        
        # Hazır veri kümelerini DB'den topla
        # Projeler
        ...
        for p in db_projects:
            try:
                ...
                if hasattr(p, "type") and p.type is not None:
                    ...
                    p_type = getattr(p.type, "value", str(p.type))
                # type alanını beklenen değerlere çevir
                if p_type in ("final", "FINAL"):
                    p_type = "final"
                elif p_type in ("interim", "INTERIM", "ara"):
                    p_type = "interim"
                projects[p.id] = {
                    "title": p.title,
                    "type": p_type or "interim",
                    "responsible_instructor_id": getattr(p, 'responsible_instructor_id', None),
                }
            except Exception:
                projects[p.id] = {
                    "title": getattr(p, "title", f"Project {p.id}"), 
                    "type": "interim",
                    "responsible_instructor_id": getattr(p, 'responsible_instructor_id', None)
                }

        # Öğretim elemanları
        db_instructors = db.query(Instructor).all()
        instructors: Dict[int, Dict[str, Any]] = {}
        for ins in db_instructors:
            role_str = (ins.role or "").lower()
            is_prof = any(k in role_str for k in ["prof", "doç", "dr. öğr"])
            instructors[ins.id] = {
                "name": ins.name,
                "type": "professor" if is_prof else "assistant",
                "role": ins.role or "",  # Role field'ını ekle
            }

        # Parametreleri isimlendirme açısından düzelt (UI farklı isimler gönderebilir)
        fixed_params = dict(params or {})
        if algorithm_name == "simulated_annealing":
            if "initial_temp" in fixed_params and "initial_temperature" not in fixed_params:
                fixed_params["initial_temperature"] = fixed_params.pop("initial_temp")
            if "min_temp" in fixed_params:
                # Şimdilik kullanılmıyor; ileride durdurma kriteri olarak eklenebilir
                fixed_params.pop("min_temp")

        # Algoritma örneğini oluştur (gelişmiş factory)
        algorithm = AlgorithmFactory.create_algorithm(
        ...
        if algorithm is None:
            ...
            algorithm_run.result = json.dumps({"error": f"Algorithm {algorithm_name} not available"})
            algorithm_run.completed_at = datetime.now()
            db.commit()
            return

        # Algoritmayı çalıştır (merkezi dedup ve duplicate raporu ile)
        start_time = time.time()
        result = algorithm.run() if hasattr(algorithm, "run") else algorithm.optimize()
        execution_time = time.time() - start_time

        # Servis katmanında ek güvence: geç (16:30+) slotları ileri değil, erken saatlere taşı ve gap'leri sıkıştır
        try:
            if isinstance(result, dict):
                ...
                for key in ("schedule", "assignments", "solution"):
                    ...
                    if isinstance(lst, list) and lst:
                        try:
                            algo_obj._compact_schedule_classrooms(lst)  # type: ignore
                        except Exception:
                            pass
                        try:
                            algo_obj._compact_schedule_globally(lst)  # type: ignore
                        except Exception:
                            pass
                        try:
                            _remove_late_assignments_backend(
                                lst,
                                getattr(algo_obj, "timeslots", []),
                                getattr(algo_obj, "classrooms", []),
                                getattr(algo_obj, "instructors", []),
                            )
                        except Exception:
                            pass
        except Exception:
        ...
        # JÜRİ ATAMASI YAP - Algoritma çalıştıktan sonra
        try:
            from app.api.v1.endpoints.algorithms import _generate_fallback_assignments
            print("Algoritma tamamlandı, jüri ataması yapılıyor...")
            _generate_fallback_assignments(db)
            print("Jüri ataması tamamlandı!")
        except Exception as e:
            print(f"Jüri atama hatası: {e}")
        
        # Update with results
        algorithm_run.status = "completed"
        algorithm_run.result = json.dumps(result)
        algorithm_run.execution_time = execution_time
        algorithm_run.completed_at = datetime.now()
        db.commit()
        ...
        try:
            ...
            if algorithm_run:
                algorithm_run.status = "failed"
                algorithm_run.result = json.dumps({"error": str(e)})
                algorithm_run.completed_at = datetime.now()
                db.commit()
        except Exception:
            pass
...
def _remove_late_assignments_backend(assignments: List[Dict[str, Any]], timeslots: List[_Any] = None, classrooms: List[_Any] = None, instructors: List[_Any] = None) -> None:
    """Backend service-level late slot enforcement (16:30+). In-place modify."""
    try:
        if not assignments:
            return
        timeslots = timeslots or []
        classrooms = classrooms or []
        def _is_late(ts: _Any) -> bool:
            try:
                s = ts.get("start_time", "") if isinstance(ts, dict) else getattr(ts, "start_time", "")
                parts = str(s).split(":")
        ...
        def _parse(ts: _Any) -> tuple:
            try:
                ...
                pass
            return (24, 0, 0)
        ts_en = list(enumerate(timeslots))
        ts_en.sort(key=lambda it: (_parse(it[1]), it[0]))
        id_to_ts: Dict[_Any, _Any] = {}
        sorted_ids: List[_Any] = []
        ts_index: Dict[_Any, int] = {}
        for _, ts in ts_en:
            tid = ts.get("id") if isinstance(ts, dict) else getattr(ts, "id", len(sorted_ids))
            id_to_ts[tid] = ts
            ts_index[tid] = len(sorted_ids)
            sorted_ids.append(tid)
        last_ok_idx = len(sorted_ids) - 1
        for i, tid in enumerate(sorted_ids):
            if _is_late(id_to_ts.get(tid)):
                last_ok_idx = i - 1
                break
        occupied: Dict[tuple, bool] = {}
        instr_busy: Dict[_Any, set] = {}
        room_ids: List[_Any] = []
        ...
        for a in assignments:
            ...
            for ins in a.get("instructors", []) or []:
                instr_busy.setdefault(ins, set()).add(idx)
        for a in list(assignments):
            if not isinstance(a, dict):
                continue
            ts = id_to_ts.get(a.get("timeslot_id"))
            if not ts or not _is_late(ts):
                continue
            ...
            if cur_idx is None:
                ...
                continue
            ok = False
            ins = a.get("instructors", []) or []
            for t_idx in range(0, max(last_ok_idx, -1) + 1):
                for cid in sorted(room_ids):
                    if occupied.get((cid, t_idx)):
                        continue
                    if all(t_idx not in instr_busy.get(i, set()) for i in ins):
                        old_cid = a.get("classroom_id")
                        if old_cid is not None:
                            occupied.pop((old_cid, cur_idx), None)
                        a["classroom_id"] = cid
                        a["timeslot_id"] = sorted_ids[t_idx]
                        occupied[(cid, t_idx)] = True
                        for i in ins:
                            instr_busy.setdefault(i, set()).discard(cur_idx)
                            instr_busy[i].add(t_idx)
                        ok = True
                        break
                if ok:
                    break
