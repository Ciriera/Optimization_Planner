...
async def init_redis_pool():
    ...
    global redis_pool
    try:
        # Redis-py ile async olmayan bağlantı kullan
        redis_pool = Redis(
            host=settings.REDIS_HOST,
            port=settings.REDIS_PORT,
            db=0,
        ...
        redis_pool.ping()  # Bağlantıyı test et
        logger.info("Redis bağlantı havuzu başlatıldı")
    except Exception as e:
        logger.error(f"Redis bağlantı havuzu oluşturulamadı: {e}")
