...
class FireflyAlgorithm(BaseAlgorithm):
    ...
    def optimize(self) -> Dict[str, Any]:
        """Run Firefly Algorithm optimization - Kurallara uygun"""
        ...
        # Evaluate fireflies with rules
        fitness_values = [self._evaluate_fitness_with_rules(f) for f in fireflies]
        
        # Main optimization loop - Firefly prensiplerine göre
        for iteration in range(self.parameters['iterations']):
            # Reduce alpha over iterations (Firefly prensibi)
            alpha = self.parameters['alpha'] * (0.97 ** iteration)
            
            # Move fireflies - Firefly çekim kuvveti
            for i in range(self.parameters['population_size']):
                for j in range(self.parameters['population_size']):
                    ...
                    if fitness_values[j] > fitness_values[i]:
                        ...
                        # Re-evaluate moved firefly with rules
                        fitness_values[i] = self._evaluate_fitness_with_rules(fireflies[i])
