...
class ParticleSwarmOptimization(BaseAlgorithm):
    ...
    def _evaluate_fitness_with_rules(self, particle):
        """Kurallara uygun fitness hesaplama"""
        fitness = 0.0
        project_ids = list(self.projects.keys())
        
        # Particle'ı çözüme çevir
        solution = self._particle_to_solution(particle)
    ...
    def _particle_to_solution(self, particle):
        """Particle'ı çözüm formatına çevir"""
        project_ids = list(self.projects.keys())
        instructor_ids = list(self.instructors.keys())
        
        solution = []
        for i in range(len(project_ids)):
            # Her proje için 3 instructor seç
            responsible_idx = int(particle[i * 3] * len(instructor_ids)) % len(instructor_ids)
            assistant1_idx = int(particle[i * 3 + 1] * len(instructor_ids)) % len(instructor_ids)
            assistant2_idx = int(particle[i * 3 + 2] * len(instructor_ids)) % len(instructor_ids)
            ...
            solution.append((responsible, assistant1, assistant2))
        
        return solution
        
    def optimize(self) -> Dict[str, Any]:
        """Run Particle Swarm Optimization algorithm - Kurallara uygun"""
        ...
        # Evaluate particles with rules
        particle_fitness = [self._evaluate_fitness_with_rules(p) for p in particles]
        ...
        for iteration in range(self.parameters['iterations']):
            ...
            for i in range(self.parameters['swarm_size']):
                ...
                # Update position - PSO formülü
                particles[i] = self._update_position(particles[i], velocities[i])
                
                # Evaluate new position with rules
                fitness = self._evaluate_fitness_with_rules(particles[i])
