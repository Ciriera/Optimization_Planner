...
def get_counts(
    *,
    ...
    """
    Tablolardaki kayıt sayılarını döndürür.
    """
    try:
        data = {
            'users': db.query(models.User).count(),
            'instructors': db.query(models.Instructor).count(),
            'projects': db.query(models.Project).count(),
            'classrooms': db.query(models.Classroom).count(),
            'timeslots': db.query(models.TimeSlot).count(),
            'schedules': db.query(models.Schedule).count(),
            'algorithm_runs': db.query(models.AlgorithmRun).count(),
        }
        # Ara tablolar (raw SQL ile)
        try:
            data['project_instructor'] = db.execute(text('SELECT COUNT(*) FROM project_instructor')).scalar() or 0
        except Exception:
            data['project_instructor'] = 0
        try:
            data['schedule_instructor'] = db.execute(text('SELECT COUNT(*) FROM schedule_instructor')).scalar() or 0
        except Exception:
            data['schedule_instructor'] = 0
